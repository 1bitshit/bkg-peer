#!/bin/bash
# Spin up a bkg-peer P2P cluster with incremental node launches.
#
# Usage:
...
CLUSTER_DIR=$(mktemp -d -t bkg-peer_cluster_XXXXXX)
PIDS=()
...
echo -e "${BOLD}=== bkg-peer P2P Cluster ===${RESET}"
...
for i in $(seq 0 $((NUM_NODES - 1))); do
    ...
    BKG_PEERD_HOME="$NODE_DIR" \
    RUST_LOG=bkg-peer=info \
    "$BIN" serve \
