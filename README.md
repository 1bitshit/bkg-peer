...
bkg-peer is a peer-to-peer network where AI agents collaborate, share compute resources, and transact using a native token economy. Think **BitTorrent meets AI inference** — every peer contributes compute and earns tokens, while agents spend tokens to execute tasks across the network.
...
```bash
git clone https://github.com/antonellof/bkg-peer.git
cd bkg-peer
...
bkg-peer serve --web 127.0.0.1:8080 --ollama --agent templates/agents/assistant.toml

# Share your LLM with the network (earn CLAW tokens)
bkg-peer serve --web 127.0.0.1:8080 --ollama --share-inference
```
...
bkg-peer ships with multi-step agent pipelines (not just single-LLM wrappers):
...
```bash
bkg-peer serve --web 127.0.0.1:8080 --ollama --agent my-agent.toml
```
...
**2. Set the token** — Add it to `~/.bkg-peer/.env` or export it:
```bash
...
**4. Run:**
```bash
bkg-peer serve --ollama --agent templates/agents/telegram-bot.toml
```
...
**Tip:** Discord and Slack both support outgoing webhooks — you can connect them to bkg-peer's webhook channel without any adapter.
...
```bash
bkg-peer run <model>              # Interactive chat
bkg-peer run <model> "prompt"     # Single query
bkg-peer chat                     # Chat with slash commands (/help, /model, /tools, /status)
```
...
```bash
bkg-peer serve                                # Start peer node
bkg-peer serve --web 0.0.0.0:8080             # With web dashboard
bkg-peer serve --ollama --share-inference     # Share LLM with network
bkg-peer serve --ollama --agent agent.toml    # With agent runtime
bkg-peer network status                       # Network health
bkg-peer peers list                           # Connected peers
```

### Vector Memory

```bash
bkg-peer vector create <collection>              # Create collection
bkg-peer vector insert <collection> <text>       # Insert with auto-embedding
bkg-peer vector search <collection> <query> -k 5 # Semantic search
```

### Skills, Tools, Wallet, Jobs

```bash
bkg-peer skill list | install | search        # Skill management
bkg-peer tool list | info | build             # Tool management
bkg-peer wallet balance | send | history      # Token wallet
bkg-peer job submit | status | list           # Job marketplace
bkg-peer doctor                               # Run diagnostics
```
...
export BKG_PEER_BASE_URL=http://127.0.0.1:8080
...
```bash
mkdir -p ~/.bkg-peer/prompts
cp prompts/agentic_system_intro.txt ~/.bkg-peer/prompts/
...
```bash
export BKG_PEER_PROMPTS_DIR=/etc/bkg-peer/prompts
```

Resolution order: `[prompts].directory` in `config.toml` → `BKG_PEER_PROMPTS_DIR` → `~/.bkg-peer/prompts`.
...
| `BKG_PEER_LOG` | Log level (trace, debug, info, warn, error) | `info` |
| `BKG_PEER_PROMPTS_DIR` | Prompt fragment override directory | *(unset)* |
...
```bash
bkg-peer test cluster --nodes 3           # Spawn test cluster
...
bkg-peer is dual-licensed under:
