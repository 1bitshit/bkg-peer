...
Agents are autonomous AI entities that run within bkg-peer. They have their own identity, budget, tools, skills, and communication channels.
...
BKG_PEER_HOME=/data/agents/research bkg-peer agent run agent.toml

# List running agents
bkg-peer agent list

# View agent logs
bkg-peer agent logs <agent-id>

# Stop agent
bkg-peer agent stop <agent-id>

# Attach to agent REPL
bkg-peer agent attach <agent-id>
```
...
```bash
bkg-peer agent attach research-bot
...
bkg-peer skill install pclaw1abc.../code-review

# Search network for skills
bkg-peer skill search "data analysis"

# Show skill details
bkg-peer skill info code-review

# Remove skill
bkg-peer skill remove code-review
```
...
|-------|--------|-------------|
| `local` | User's `~/.bkg-peer/skills/` | All tools |
...
bkg-peer tool build ./my-tool --output ./tools/my-tool.wasm

# Install WASM tool
bkg-peer tool install ./tools/my-tool.wasm
...
embedding_dim = 384
auto_persist = true
persistence_path = "~/.bkg-peer/vector/agent-memory"
```
...
bkg-peer vector create agent-memory

# Insert memory
bkg-peer vector insert agent-memory "Important fact to remember"

# Search memories
bkg-peer vector search agent-memory "what does user prefer" -k 5
```
...
```bash
bkg-peer agent run templates/flows/deep-researcher.toml
```
...
| **bkg-peerBot** | `telegram-bot.toml` | Telegram bot with web search, powered by local inference |
...
bkg-peer agent run templates/agents/researcher.toml

# Deploy a Telegram bot (set token first)
echo 'TELEGRAM_BOT_TOKEN=your_token_here' >> ~/.bkg-peer/.env
bkg-peer agent run templates/agents/telegram-bot.toml

# Deploy a network monitor with scheduled checks
bkg-peer agent run templates/agents/monitor.toml

# Check running agents
bkg-peer agent list

# View agent details
bkg-peer agent info <agent-id>
```
...
7. Deploy: `bkg-peer agent run my-agent.toml`
