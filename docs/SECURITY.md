...
bkg-peer implements defense-in-depth security across multiple layers: sandboxed execution, credential protection, encryption, and runtime safety checks.
...
|-------------|--------|--------------|
| **Local** | User-created in `~/.bkg-peer/skills/` | Full tool access |
...
enabled = true
level = "info"
output = "~/.bkg-peer/logs/security.log"
max_size_mb = 100
rotate_count = 5
```
...
5. **Update regularly** — Keep bkg-peer updated for security patches
