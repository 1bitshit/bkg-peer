//! `bkg-peer wallet` commands - Wallet operations.
...
fn load_wallet() -> anyhow::Result<(Wallet, NodeIdentity)> {
    ...
    if !identity_path.exists() {
        anyhow::bail!("No wallet found. Run 'bkg-peer wallet create' first.");
    }
...
pub async fn run(cmd: WalletCommand) -> anyhow::Result<()> {
    match cmd {
        ...
        WalletCommand::Info => {
            ...
            if !identity_path.exists() {
                println!("No wallet found. Run 'bkg-peer wallet create' to create one.");
                return Ok(());
            }
