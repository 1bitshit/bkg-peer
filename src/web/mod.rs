...
pub struct WebState {
    ...
    /// Tool registry from the node (`bkg-peer serve`); powers agentic chat ReAct loop.
    ...
    /// Wired on `bkg-peer serve` for model downloads and `/api/inference/settings`.
...
async fn api_skills_studio_ai(
    ...
    let system_rules = r#"You help edit bkg-peer SKILL.md files.
...
/// Routes dashboard inference through the same `mpsc` queue as `bkg-peer serve`.
...
async fn api_chat(
    ...
    Json(ChatResponse {
        response: format!(
            "Chat is available via CLI: bkg-peer chat --model {}\n\n\
...
async fn api_inference_settings_get(
    ...
    let Some(inf) = &state.inference else {
        return Err((
            StatusCode::SERVICE_UNAVAILABLE,
            "Inference engine not attached (run bkg-peer serve with --web).".into(),
        ));
    };
...
async fn api_create_task(
    ...
    if let Some(tx) = &state.agent_task_tx {
        ...
        if let Some(t) = tasks.iter_mut().find(|t| t.id == task_id) {
            ...
            t.completed_at = Some(chrono::Utc::now().to_rfc3339());
            t.result = Some(
                "MCP mode requires inference and at least one connected MCP tool. Without MCP, use a full `bkg-peer serve` node (agentic tools) or `--agent` for a spec-driven runtime."
                    .to_string(),
            );
            t.logs.push(format!(
        ...
        if let Some(t) = tasks.iter_mut().find(|t| t.id == task_id) {
            ...
            t.completed_at = Some(chrono::Utc::now().to_rfc3339());
            t.result = Some(
                "No execution path for this task. Use `bkg-peer serve` with inference, or `--agent` for a spec-driven agent, or enable MCP in settings when tools are connected."
                    .to_string(),
            );
            t.logs.push(format!(
...
async fn api_test_channel(
    ...
    let test_message = crate::messaging::ChannelMessage::response(
        channel_id.clone(),
        "bkg-peer test message".to_string(),
    );
